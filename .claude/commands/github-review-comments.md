...
gh api "repos/mhenrixon/phlex-forms/pulls/<PR_NUMBER>/comments" --paginate
...
' -f owner=mhenrixon -f repo=phlex-forms -F pr=<PR_NUMBER>
```
...
```bash
gh api "repos/mhenrixon/phlex-forms/pulls/<PR>/comments/<COMMENT_ID>/replies" \
  --method POST \
